...
void World::Update(Snake& l_player, Textbox& l_textbox) {
  if (l_player.GetPosiion() == m_item) {
    l_player.Extend();
    l_player.IncreaseScore();
    l_textbox.Add("You ate an apple. Score: " +
                  std::to_string(l_player.GetScore()));
    RespawnApple();
  }
  ...
  if (l_player.GetPosiion().x <= 0 || l_player.GetPosiion().y <= 0 ||
      l_player.GetPosiion().x >= gridSize_x - 1 ||
      l_player.GetPosiion().y >= gridSize_y - 1) {
